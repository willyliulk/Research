...
import flax
from env.jax_env_target_3d import EnvParams, EnvState, reset_env, step_env
from models.target_ppo_network import TargetActorCritic
import numpy as np

def load_params(path, network):
    # 需要給 Dummy 觀測值來初始化架構
    dummy_obs = {
    ...
    with open(path, "rb") as f:
        return flax.serialization.from_bytes(init_params, f.read())

def evaluate_optimized():
    network = TargetActorCritic(action_dim=2)
    try:
        params = load_params("jaxTraining/checkpoints/checkpoint_20260707-1951/model.msgpack", network)
    except:
        params = load_params("jaxTraining/checkpoints/checkpoint_20260707-1951/model.msgpack", network)
    ...
    init_carry = {
        ...
        "final_metrics": {
            ...
            "tar_crash": jnp.zeros(num_envs, dtype=jnp.int32),
            "timeout": jnp.zeros(num_envs, dtype=jnp.int32),
            "out_of_bounds": jnp.zeros(num_envs, dtype=jnp.int32),
    ...
    def scan_step(carry, _):
        ...
        next_metrics = {
            ...
            "tar_crash": update_metric(carry["final_metrics"]["tar_crash"], info["tar_crash"]),
            "timeout": update_metric(carry["final_metrics"]["timeout"], info["timeout"]),
            "out_of_bounds": update_metric(carry["final_metrics"]["out_of_bounds"], info["out_of_bounds"]),
            "episode_len": update_metric(carry["final_metrics"]["episode_len"], info["episode_len"]),
    ...
    print(f"UAV Hits (Target intercepted): {int(jnp.sum(metrics['hit']))} ({(jnp.sum(metrics['hit'])/num_envs)*100:.1f}%)")
    print(f"Target Crashes (Ocean): {int(jnp.sum(metrics['tar_crash']))} ({(jnp.sum(metrics['tar_crash'])/num_envs)*100:.1f}%)")
    print(f"Timeouts (Target Evaded): {int(jnp.sum(metrics['timeout']))} ({(jnp.sum(metrics['timeout'])/num_envs)*100:.1f}%)")
    print(f"Out of bounds: {int(jnp.sum(metrics['out_of_bounds']))} ({(jnp.sum(metrics['out_of_bounds'])/num_envs)*100:.1f}%)")
    print(f"Avg Survive Steps: {np.mean(metrics['episode_len']):.1f}")
    ...
    final_crashes = np.array(jax.device_get(metrics['tar_crash']))
    final_timeouts = np.array(jax.device_get(metrics['timeout']))
    final_oob = np.array(jax.device_get(metrics['out_of_bounds']))
    final_episode_len = np.array(jax.device_get(metrics['episode_len']))
    final_reward = np.array(jax.device_get(metrics['reward']))
    
    status_list = []
    for h, c, t, o in zip(final_hits, final_crashes, final_timeouts, final_oob):
        if h: status_list.append("Hit")
        elif c: status_list.append("Crash")
        elif t: status_list.append("Timeout")
        elif o: status_list.append("Out of Bounds")
        else: status_list.append("Unknown")
